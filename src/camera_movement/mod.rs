...
impl<T: cgmath::BaseFloat> OrbitCamera<T> {
    ...
    pub fn update(&mut self, cur_mouse: Vector2<T>) {
        if self.rotating {
            let prev_pt = OrbitCamera::get_vec_on_ball(self.prev_mouse);
            let cur_pt = OrbitCamera::get_vec_on_ball(cur_mouse);
            ...
            let mouse_vec = -(cur_mouse - self.prev_mouse).normalize_to(self.pan_speed);
            ...
            let mouse_dist = -(cur_mouse - self.prev_mouse).normalize_to(self.zoom_speed).magnitude();
            self.distance = (self.distance + mouse_dist).max(T::zero());
            self.prev_mouse = cur_mouse;
        }
    }
}
